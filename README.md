...
旅程ごとにホテル・交通手段・費用を記録して比較するための個人用ポータルサイトです。
データの保存先はCloudflare自身が提供する **KV**(シンプルなキーバリューストア)。Notionもスプレッドシートも不要です。

**主な機能**
- 旅程ごとの管理(目的地・日程・人数・予算・自由メモ)
- 宿泊候補の記録・比較・決定マーク
- 交通手段の記録(新幹線・飛行機・電車・バスなど、区間・出発/到着日時・料金・決定マーク)
- その他費用の記録(食事・観光・お土産など)
- 旅費サマリー(宿泊費+交通費+その他費用の合計を自動計算し、予算と比較)
...
- `src/index.js` — メインのWorker。認証チェック → APIルーティング(旅程/宿泊/交通/費用) → 該当しなければ`public/`の静的ファイルを返す
